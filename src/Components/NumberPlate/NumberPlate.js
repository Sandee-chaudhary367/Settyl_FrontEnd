...
let NumberPlate=({title,val,arr})=>{
    ...
    let HandleChange=async(e)=>{
        ...
        const response=await axios.post("http://localhost:3001/getCount",{[name]:value});
    ...
    return(
      ...
      <select  style={{width:"150px"}}  onChange={HandleChange} className="dd" id={`${title.toLowerCase()}123`} name={title}>
       {arr.map((val)=> {
        if(val===""){
                return;
        }
